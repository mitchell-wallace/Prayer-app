...
const DURATION_OFFSETS: Record<DurationPreset, { days?: number; months?: number; years?: number }> = {
  '10d': { days: 10 },
  '1m': { months: 1 },
  '3m': { months: 3 },
  '6m': { months: 6 },
...
function computeExpiry(createdAt: number, preset: DurationPreset): number {
  const base = new Date(createdAt);
  const offset = DURATION_OFFSETS[preset] || DURATION_OFFSETS['6m'];
...
function createSchemaV2(db: DbInstance): void {
  db.exec(`
...
function migrateV1ToV2(db: DbInstance): void {
  db.run('BEGIN');
  try {
